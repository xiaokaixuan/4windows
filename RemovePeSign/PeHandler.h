...
typedef BOOL (WINAPI *Fun_GetFileSizeEx)(HANDLE hFile, PLARGE_INTEGER lpFileSize);

__forceinline Fun_GetFileSizeEx GetPGetFileSizeEx()
{
	static Fun_GetFileSizeEx pGetFileSizeEx = 
		(Fun_GetFileSizeEx)GetProcAddress(LoadLibraryW(L"kernel32.dll"), "GetFileSizeEx");
...
__forceinline LPVOID MapPeFile(LPCTSTR pwzFile, PLARGE_INTEGER lpFileSize)
{
	if (lstrlen(pwzFile) <= 0) // 如果传入的文件路径为空
		return NULL;
	...
	GetPGetFileSizeEx()(hFile, lpFileSize);
...
__forceinline BOOL RemovePeSign(LPVOID pBase, PLARGE_INTEGER lpFileSize, LPCTSTR pwzNewFile)
{
	BOOL bRet = FALSE;
